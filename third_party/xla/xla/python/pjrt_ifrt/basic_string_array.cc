...
BasicStringArray::DisassembleIntoSingleDeviceArrays(
  ...
  // For each single device array we are going to create, pre-make:
  ...
  //
  // The Futures and the on-done-with-host-buffer `callbacks, shapes and
  // shardings are used to make the arrays. The promises, the buffer backing
  // stores are passed onto the OnReady callback that populates them when the
  // buffers of the source array become ready.
  ...
  struct PerShardBufferBackingStore {  // Data (strings) for a single shard.
    void CopyFrom(absl::Span<const absl::string_view> input_buffer) {
      ...
      for (int i = 0; i < input_buffer.size(); ++i) {
        strings.push_back(
            std::string(input_buffer[i].data(), input_buffer[i].size()));
      }
