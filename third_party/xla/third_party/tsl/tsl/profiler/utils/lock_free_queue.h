...
//    * A normal BlockedQueue which do not concerning the concurrency
//
...
// case, or it could be size_t if no concurrence need to be worry for it. In
...
//   * BlockedQueue support move constrctor/assignment and iterators
//
...
template <>
struct SizeAtomic<false> {
  size_t value;
  explicit SizeAtomic(size_t pos = 0) : value(pos) {}
  size_t Get() const { return value; }
  void Set(size_t pos) { value = pos; }
};

template <>
struct SizeAtomic<true> {
...
template <typename T, typename TSizeAtomic, size_t kBlockSize>
class BlockedQueueBase {
 ...
 protected:
  ...
  TSizeAtomic end_;     // Atomic: read also by consumer thread.
};
...
class BlockedQueue final
    : public QueueBaseInternal::BlockedQueueBase<
          T, QueueBaseInternal::SizeAtomic<false>, kBlockSize> {
...
class LockFreeQueue final
    : public QueueBaseInternal::BlockedQueueBase<
          T, QueueBaseInternal::SizeAtomic<true>, kBlockSize> {
  using Block = QueueBaseInternal::Block<T, kBlockSize>;
